...
export default function RbacPage() {
  ...
  return ( <div className="space-y-6"> {/* Toast Notification */}
      ...
      <ModuleBanner
        badge={{
          label: 'Hệ Thống Phân Quyền & Kiểm Soát Phạm Vi Dữ Liệu',
          icon: ShieldCheck,
          variant: 'indigo',
        }}
        title="Cấu Hình Phân Quyền Truy Cập & RBAC"
        subtitle="Cài đặt ma trận 12 quyền hạn chi tiết và phạm vi dữ liệu (Data Scope) an toàn theo từng chức danh & phòng ban"
        kpis={[
          { label: 'Chức Danh Hệ Thống', value: `${roleMatrix.length} Chức Danh`, subtext: 'Liên thông HRM' },
          { label: 'Quyền Hạn Cốt Lõi', value: '12 Quyền Hạn', subtext: 'Granular Matrix' },
          { label: 'Phạm Vi Dữ Liệu', value: '4 Cấp Boundary', subtext: 'ALL / DEPT / TEAM / OWN' },
        ]}
        actions={
          <div className="flex items-center gap-2 flex-wrap">
            <button
              onClick={() => setIsCreateModalOpen(true)}
              className="px-3.5 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 dark:bg-indigo-950/40 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-colors"
            >
            ...
            <button
              ...
              className="px-3.5 py-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg text-xs flex items-center gap-1.5 shadow-xs transition-colors"
            >
