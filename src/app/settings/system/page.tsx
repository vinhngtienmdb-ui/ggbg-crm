...
export default function SystemSettingsPage() {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      {/* Toast Notification */}
      {saveToast && (
        <div className="p-4 rounded-2xl bg-success-fg text-white font-bold text-xs shadow-cardLg flex items-center justify-between animate-in fade-in slide-in-from-top duration-300">
          <div className="flex items-center gap-2">
            <CheckCircle2 className="w-5 h-5 text-success-dot" />
            <span>{saveToast}</span>
          </div>
        </div>
      )}
      ...
      {/* Header Section */}
      <div className="bg-white p-6 rounded-2xl border border-line shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-xl font-bold text-ink-900">Cấu Hình Tích Hợp Hệ Thống</h1>
          </div>
        ...
        <button
          ...
          className="px-5 py-2.5 bg-brand-600 hover:bg-brand-700 text-white font-bold rounded-xl text-xs flex items-center gap-2 shadow-lg shadow-brand-600/20 transition-all active:scale-95 shrink-0"
        >
      ...
      {/* Security Notice */}
      <div className="p-4 rounded-2xl bg-gradient-to-r from-brand-50 via-white to-white text-white border border-line shadow-md flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-success-dot/20 border border-success-border/30 flex items-center justify-center text-success-dot shrink-0">
        ...
        <span className="px-3 py-1 bg-success-dot/20 border border-success-border/30 text-success-dot rounded-full text-[10px] font-bold shrink-0 hidden sm:inline-block">
          ✓ Đã Mã Hóa An Toàn
        </span>
      </div>

      {/* CONFIGURATION TABS NAVIGATION */}
      <div className="flex items-center gap-2 bg-white p-2 rounded-2xl border border-line overflow-x-auto touch-scroll sleek-scrollbar">
        <button
          onClick={() => setActiveTab('MODULE_TOGGLES')}
          className={`px-4 py-2.5 rounded-xl text-xs font-bold flex items-center gap-2 transition-all ${
            activeTab === 'MODULE_TOGGLES' ? 'bg-brand-600 text-white shadow-sm' : 'text-ink-500 hover:bg-line-soft'
          }`}
        >
          <Sliders className="w-4 h-4 text-plum-deep" /> 🎛️ Phân Hệ Tính Năng
        </button>

        <button
          onClick={() => setActiveTab('INFRASTRUCTURE')}
          className={`px-4 py-2.5 rounded-xl text-xs font-bold flex items-center gap-2 transition-all ${
            activeTab === 'INFRASTRUCTURE' ? 'bg-brand-600 text-white shadow-sm' : 'text-ink-500 hover:bg-line-soft'
          }`}
        >
          <Cloud className="w-4 h-4 text-warn-fg" /> ☁️ Hạ Tầng Cloud & DB
        </button>

        <button
          onClick={() => setActiveTab('API_KEYS')}
          className={`px-4 py-2.5 rounded-xl text-xs font-bold flex items-center gap-2 transition-all ${
            activeTab === 'API_KEYS' ? 'bg-brand-600 text-white shadow-sm' : 'text-ink-500 hover:bg-line-soft'
          }`}
        >
          <Key className="w-4 h-4 text-warn-fg" /> 🔌 Sàn TMĐT & AI API Keys
        </button>

        <button
          onClick={() => setActiveTab('SMTP')}
          className={`px-4 py-2.5 rounded-xl text-xs font-bold flex items-center gap-2 transition-all ${
            activeTab === 'SMTP' ? 'bg-brand-600 text-white shadow-sm' : 'text-ink-500 hover:bg-line-soft'
          }`}
        >
          <Mail className="w-4 h-4 text-brand-400" /> 📧 Máy Chủ Email SMTP
        </button>

        <button
          onClick={() => setActiveTab('WEBHOOKS')}
          className={`px-4 py-2.5 rounded-xl text-xs font-bold flex items-center gap-2 transition-all ${
            activeTab === 'WEBHOOKS' ? 'bg-brand-600 text-white shadow-sm' : 'text-ink-500 hover:bg-line-soft'
          }`}
        >
          <Bell className="w-4 h-4 text-plum-deep" /> 🔔 Webhook & Telegram/Zalo
        </button>

        <button
          onClick={() => setActiveTab('SECURITY_AUDIT')}
          className={`px-4 py-2.5 rounded-xl text-xs font-bold flex items-center gap-2 transition-all ${
            activeTab === 'SECURITY_AUDIT' ? 'bg-brand-600 text-white shadow-sm' : 'text-ink-500 hover:bg-line-soft'
          }`}
        >
          <ShieldAlert className="w-4 h-4 text-success-dot" /> 🛡️ Bảo Mật & Audit Logs
        </button>
      </div>
      ...
      <form onSubmit={(e) => handleSave(e, `Lưu cấu hình tab ${activeTab}`)} className="space-y-6">
        ...
        {activeTab === 'MODULE_TOGGLES' && (
          <div className="space-y-5">
            ...
            {isAdmin ? (
              <div className="p-4 rounded-xl bg-brand-50 border border-brand-100 text-brand-900 text-xs font-medium flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  <ShieldCheck className="w-5 h-5 text-brand-600 shrink-0" />
                  <div>
                    <p className="font-bold text-brand-900">Quyền Quản Trị Viên (Admin Privileges Active)</p>
                ...
                <span className="px-2.5 py-1 bg-brand-600 text-white rounded-lg text-[10px] font-bold shrink-0">
                  SYSTEM-WIDE ADMIN
                </span>
              </div>
            ) : (
              <div className="p-4 rounded-xl bg-warn-bg border border-gold-border text-warn-fg text-xs font-medium flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  <AlertTriangle className="w-5 h-5 text-warn-fg shrink-0" />
                  <div>
                    <p className="font-bold text-warn-fg">Quyền Hạn Hạn Chế (Chỉ Xem)</p>
                ...
                <span className="px-2.5 py-1 bg-warn-fg text-white rounded-lg text-[10px] font-bold shrink-0">
                  READ-ONLY
                </span>
              </div>
            )}

            <div className="bg-white p-5 rounded-lg border border-line flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
                <h3 className="font-bold text-ink-900 text-sm flex items-center gap-2">
                  <Sliders className="w-4 h-4 text-plum-fg" /> Quản Lý Phân Hệ Tính Năng (Module Feature Toggles)
                </h3>
              ...
              <button
                ...
                className={`px-3 py-1.5 font-semibold rounded text-xs border transition-all ${
                  isAdmin
                    ? 'bg-line-soft hover:bg-line text-ink-700 border-line cursor-pointer'
                    : 'bg-surface-subtle text-ink-400 border-line cursor-not-allowed opacity-60'
                }`}
              >
                ...
                return (
                  <div
                    ...
                    className={`p-4 rounded-lg border transition-all flex items-center justify-between gap-4 ${
                      isEnabled ? 'bg-white border-line ' : 'bg-surface-subtle border-line opacity-60'
                    }`}
                  >
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-ink-900 text-xs">{mod.name}</span>
                        <span className="text-[10px] font-mono text-brand-600 bg-brand-50 px-1.5 py-0.5 rounded border border-brand-100">
                          {mod.path}
                        </span>
                      </div>
                      <p className="text-[11px] text-ink-500 mt-1">{mod.desc}</p>
                    </div>
                    ...
                    <button
                      ...
                      } ${isEnabled ? 'bg-brand-600' : 'bg-line-muted'}`}
                    >
                      <span
        ...
        {activeTab === 'INFRASTRUCTURE' && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-line shadow-sm overflow-hidden">
              <div className="p-5 border-b border-line bg-surface-subtle flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-warn-bg border border-gold-border flex items-center justify-center text-warn-fg">
                    <Cloud className="w-5 h-5" />
                  </div>
                  <div>
                    <h3 className="font-bold text-sm text-ink-900 flex items-center gap-2">
                      Cloudflare Object Storage
                      <span className="px-2 py-0.5 bg-warn-bg text-warn-fg rounded text-[10px] font-bold">PDF & Documents</span>
                    </h3>
                    <p className="text-[11px] text-ink-500">Lưu trữ tập tin Hợp đồng lao động, Ghi âm cuộc gọi và File chứng từ KYC</p>
                  </div>
                </div>
              ...
              <div className="p-6 space-y-4">
                {r2Test.status !== 'IDLE' && (
                  <div className={`p-4 rounded-xl text-xs font-semibold border flex items-center justify-between ${
                    r2Test.status === 'SUCCESS' ? 'bg-success-bg border-success-border text-success-fg' : 'bg-danger-bg border-danger-border text-danger-fg'
                  }`}>
                    <div className="flex items-center gap-2">
                      {r2Test.status === 'SUCCESS' ? <CheckCircle2 className="w-4 h-4 text-success-fg" /> : <AlertTriangle className="w-4 h-4 text-danger-fg" />}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-ink-700 mb-1">Cloudflare Account ID</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs font-bold text-ink-700 mb-1">Bucket Name</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs font-bold text-ink-700 mb-1">Access Key ID</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs font-bold text-ink-700 mb-1">Custom Domain Endpoint (Ẩn Khỏi Giao Diện)</label>
                    <input
                      type="text"
            ...
            <div className="bg-white rounded-2xl border border-line shadow-sm overflow-hidden">
              <div className="p-5 border-b border-line bg-surface-subtle flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-success-bg border border-success-border flex items-center justify-center text-success-fg">
                    <Database className="w-5 h-5" />
                  </div>
                  <div>
                    <h3 className="font-bold text-sm text-ink-900 flex items-center gap-2">
                      Cơ Sở Dữ Liệu Supabase PostgreSQL
                      <span className="px-2 py-0.5 bg-success-bg text-success-fg rounded text-[10px] font-bold">Database Server</span>
                    </h3>
                    <p className="text-[11px] text-ink-500">Lưu trữ 10+ Bảng dữ liệu Khách hàng, Lead, Nhân sự, KPIs, RBAC & Audit Logs</p>
                  </div>
                </div>
              ...
              <div className="p-6 space-y-4">
                {supabaseTest.status !== 'IDLE' && (
                  <div className={`p-4 rounded-xl text-xs font-semibold border flex items-center justify-between ${
                    supabaseTest.status === 'SUCCESS' ? 'bg-success-bg border-success-border text-success-fg' : 'bg-danger-bg border-danger-border text-danger-fg'
                  }`}>
                    <div className="flex items-center gap-2">
                      {supabaseTest.status === 'SUCCESS' ? <CheckCircle2 className="w-4 h-4 text-success-fg" /> : <AlertTriangle className="w-4 h-4 text-danger-fg" />}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-ink-700 mb-1">Project URL</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs font-bold text-ink-700 mb-1">Database Name</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs font-bold text-ink-700 mb-1">Anon Key (Masked)</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs font-bold text-ink-700 mb-1">Max Connections Pool</label>
                    <input
                      type="number"
            ...
            <div className="bg-white rounded-2xl border border-line shadow-sm overflow-hidden">
              <div className="p-5 border-b border-line bg-surface-subtle flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-sm text-ink-900 flex items-center gap-2">
                      ...
                      <span className="px-2 py-0.5 bg-brand-300 text-brand-800 rounded text-[10px] font-bold">Telephony Gateway</span>
                    </h3>
                    <p className="text-[11px] text-ink-500">Tích hợp gọi điện WebRTC Softphone trực tiếp từ trình duyệt web CRM</p>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="px-4 py-2 bg-brand-50 text-brand-800 hover:bg-brand-300 border border-brand-100 rounded-xl text-xs font-bold flex items-center gap-2 transition-all active:scale-95 disabled:opacity-50"
                >
                  <Activity className={`w-4 h-4 ${voipTest.status === 'TESTING' ? 'animate-spin text-brand-600' : 'text-brand-600'}`} />
              ...
              <div className="p-6 space-y-4">
                {voipTest.status !== 'IDLE' && (
                  <div className={`p-4 rounded-xl text-xs font-semibold border flex items-center justify-between ${
                    voipTest.status === 'SUCCESS' ? 'bg-success-bg border-success-border text-success-fg' : 'bg-danger-bg border-danger-border text-danger-fg'
                  }`}>
                    <div className="flex items-center gap-2">
                      {voipTest.status === 'SUCCESS' ? <CheckCircle2 className="w-4 h-4 text-success-fg" /> : <AlertTriangle className="w-4 h-4 text-danger-fg" />}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-ink-700 mb-1">Nhà Cung Cấp Tổng Đài</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs font-bold text-ink-700 mb-1">API Endpoint Gateway</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs font-bold text-ink-700 mb-1">API Key / Token (Masked)</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs font-bold text-ink-700 mb-1">Thời Gian Lưu Ghi Âm (Ngày)</label>
                    <input
                      type="number"
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-2xl border border-line shadow-sm p-6 space-y-6">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-line pb-4">
                <div>
                  <h3 className="font-bold text-sm text-ink-900 flex items-center gap-2">
                    <ShoppingBag className="w-5 h-5 text-warn-fg" /> Cấu Hình API Keys Sàn TMĐT & AI Assistant
                  </h3>
              ...
              {apiKeysTest.status !== 'IDLE' && (
                <div className={`p-4 rounded-xl text-xs font-semibold border flex items-center justify-between ${
                  apiKeysTest.status === 'SUCCESS' ? 'bg-success-bg border-success-border text-success-fg' : 'bg-danger-bg border-danger-border text-danger-fg'
                }`}>
                  <div className="flex items-center gap-2">
                    {apiKeysTest.status === 'SUCCESS' ? <CheckCircle2 className="w-4 h-4 text-success-fg" /> : <AlertTriangle className="w-4 h-4 text-danger-fg" />}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {/* Shopee */}
                <div className="p-4 bg-surface-subtle border border-line rounded-2xl space-y-3">
                  <span className="font-bold text-xs text-warn-fg flex items-center gap-1.5">
                    🛍️ Shopee Open Platform API
                  </span>
                  <div>
                    <label className="block text-[11px] font-semibold text-ink-700 mb-1">Shopee App Key</label>
                    <input
                      type="text"
                      value={config.api_keys.shopee_app_key}
                      onChange={(e) => setConfig({ ...config, api_keys: { ...config.api_keys, shopee_app_key: e.target.value } })}
                      className="w-full p-2 bg-white border border-line rounded-xl text-xs font-mono text-ink-900"
                    />
                  </div>
                  <div>
                    <label className="block text-[11px] font-semibold text-ink-700 mb-1">Shopee App Secret</label>
                    <div className="relative">
                      <input
                        ...
                        className="w-full p-2 pr-9 bg-white border border-line rounded-xl text-xs font-mono text-ink-900"
                      />
                      <button
                        type="button"
                ...
                <div className="p-4 bg-surface-subtle border border-line rounded-2xl space-y-3">
                  ...
                  <div>
                    <label className="block text-[11px] font-semibold text-ink-700 mb-1">TikTok App Key</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-[11px] font-semibold text-ink-700 mb-1">TikTok App Secret</label>
                    <div className="relative">
                      <input
                ...
                {/* Lazada */}
                <div className="p-4 bg-surface-subtle border border-line rounded-2xl space-y-3">
                  <span className="font-bold text-xs text-brand-600 flex items-center gap-1.5">
                    🔷 Lazada Open Platform API
                  </span>
                  <div>
                    <label className="block text-[11px] font-semibold text-ink-700 mb-1">Lazada App Key</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-[11px] font-semibold text-ink-700 mb-1">Lazada App Secret</label>
                    <div className="relative">
                      <input
                ...
                <div className="p-4 bg-surface-subtle border border-line rounded-2xl space-y-3">
                  ...
                  <div>
                    <label className="block text-[11px] font-semibold text-ink-700 mb-1">Seller ID</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-[11px] font-semibold text-ink-700 mb-1">LWA Client ID</label>
                    <input
                      type="text"
                ...
                {/* AI Assistant Key */}
                <div className="p-4 bg-plum-bg/70 border border-plum-border rounded-2xl space-y-3 md:col-span-2">
                  <span className="font-bold text-xs text-plum-fg flex items-center gap-1.5">
                    ✨ Trợ Lý AI Antigravity & Gemini Engine Key
                  </span>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[11px] font-semibold text-plum-fg mb-1">Google Gemini API Key</label>
                      <div className="relative">
                        <input
                          ...
                          className="w-full p-2 pr-9 bg-white border border-plum-border rounded-xl text-xs font-mono text-plum-fg"
                        />
                        <button
                          type="button"
                          onClick={() => toggleShowKey('gemini')}
                          className="absolute right-2.5 top-1/2 -translate-y-1/2 text-plum-deep hover:text-plum-fg"
                        >
                    ...
                    <div>
                      <label className="block text-[11px] font-semibold text-plum-fg mb-1">OpenAI API Key (Fallback)</label>
                      <div className="relative">
                        <input
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-2xl border border-line shadow-sm p-6 space-y-6">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-line pb-4">
                <div>
                  <h3 className="font-bold text-sm text-ink-900 flex items-center gap-2">
                    <Mail className="w-5 h-5 text-brand-600" /> Cấu Hình Máy Chủ Email SMTP
                  </h3>
                  <p className="text-[11px] text-ink-500 mt-0.5">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-brand-50 text-brand-800 hover:bg-brand-300 border border-brand-100 rounded-xl text-xs font-bold flex items-center gap-2 transition-all active:scale-95 shrink-0 disabled:opacity-50"
                >
                  <Send className={`w-4 h-4 ${smtpTest.status === 'TESTING' ? 'animate-spin text-brand-600' : 'text-brand-600'}`} />
              ...
              {smtpTest.status !== 'IDLE' && (
                <div className={`p-4 rounded-xl text-xs font-semibold border flex items-center justify-between ${
                  smtpTest.status === 'SUCCESS' ? 'bg-success-bg border-success-border text-success-fg' : 'bg-danger-bg border-danger-border text-danger-fg'
                }`}>
                  <div className="flex items-center gap-2">
                    {smtpTest.status === 'SUCCESS' ? <CheckCircle2 className="w-4 h-4 text-success-fg" /> : <AlertTriangle className="w-4 h-4 text-danger-fg" />}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="md:col-span-2">
                  <label className="block text-xs font-bold text-ink-700 mb-1">SMTP Server Host *</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-bold text-ink-700 mb-1">Port & Mã Hóa *</label>
                  <div className="flex gap-2">
                    <input
                      type="number"
                      value={config.smtp.port}
                      onChange={(e) => setConfig({ ...config, smtp: { ...config.smtp, port: Number(e.target.value) } })}
                      className="w-24 p-2.5 bg-surface-subtle border border-line rounded-xl text-xs font-mono text-ink-900"
                    />
                    <select
                ...
                <div>
                  <label className="block text-xs font-bold text-ink-700 mb-1">SMTP Username *</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-bold text-ink-700 mb-1">SMTP Password (Masked) *</label>
                  <div className="relative">
                    <input
                      ...
                      className="w-full p-2.5 pr-9 bg-surface-subtle border border-line rounded-xl text-xs font-mono text-ink-900"
                      required
                    />
                    <button
                      type="button"
                ...
                <div>
                  <label className="block text-xs font-bold text-ink-700 mb-1">Sender Email *</label>
                  <input
                    type="email"
                ...
                <div className="md:col-span-3">
                  <label className="block text-xs font-bold text-ink-700 mb-1">Tên Người Gửi Hiển Thị (Sender Display Name)</label>
                  <input
                    type="text"
                    value={config.smtp.sender_name}
                    onChange={(e) => setConfig({ ...config, smtp: { ...config.smtp, sender_name: e.target.value } })}
                    className="w-full p-2.5 bg-surface-subtle border border-line rounded-xl text-xs text-ink-900"
                  />
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-2xl border border-line shadow-sm p-6 space-y-6">
              <div className="flex items-center justify-between border-b border-line pb-4">
                <div>
                  <h3 className="font-bold text-sm text-ink-900 flex items-center gap-2">
                    <Bell className="w-5 h-5 text-plum-fg" /> Cấu Hình Webhook & Telegram/Zalo Bot
                  </h3>
                  <p className="text-[11px] text-ink-500 mt-0.5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div className="p-4 bg-surface-subtle border border-line rounded-2xl space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="px-3 py-1 bg-brand-300 text-brand-800 hover:bg-brand-200 font-bold rounded-lg text-[10px] flex items-center gap-1"
                    >
                  ...
                  {telegramTest.status !== 'IDLE' && (
                    <p className={`text-[11px] font-semibold ${telegramTest.status === 'SUCCESS' ? 'text-success-fg' : 'text-danger-fg'}`}>
                      {telegramTest.message}
                    </p>
                  )}

                  <div>
                    <label className="block text-[11px] font-semibold text-ink-700 mb-1">Telegram Bot Token</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-[11px] font-semibold text-ink-700 mb-1">Telegram Chat ID / Group ID</label>
                    <input
                      type="text"
                ...
                <div className="p-4 bg-surface-subtle border border-line rounded-2xl space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-xs text-brand-800 flex items-center gap-1.5">
                  ...
                  {zaloTest.status !== 'IDLE' && (
                    <p className={`text-[11px] font-semibold ${zaloTest.status === 'SUCCESS' ? 'text-success-fg' : 'text-danger-fg'}`}>
                      {zaloTest.message}
                    </p>
                  )}

                  <div>
                    <label className="block text-[11px] font-semibold text-ink-700 mb-1">Zalo ZNS Endpoint URL</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-[11px] font-semibold text-ink-700 mb-1">Zalo Official Account (OA) App ID</label>
                    <input
                      type="text"
                ...
                <div className="p-4 bg-plum-bg/70 border border-plum-border rounded-2xl space-y-3 md:col-span-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-1">
                    <label className="p-3 bg-white border border-plum-border rounded-xl flex items-center gap-3 cursor-pointer">
                      <input
                        type="checkbox"
                        ...
                        className="w-4 h-4 accent-plum-fg rounded"
                      />
                      <span className="text-xs font-bold text-ink-900">🎯 Có Lead Mới Về Phễu</span>
                    </label>
                    ...
                    <label className="p-3 bg-white border border-plum-border rounded-xl flex items-center gap-3 cursor-pointer">
                      <input
                        ...
                        className="w-4 h-4 accent-plum-fg rounded"
                      />
                      <span className="text-xs font-bold text-ink-900">⏰ Hạn Chót Chấm KPI / 360°</span>
                    </label>
                    ...
                    <label className="p-3 bg-white border border-plum-border rounded-xl flex items-center gap-3 cursor-pointer">
                      <input
                        ...
                        className="w-4 h-4 accent-plum-fg rounded"
                      />
                      <span className="text-xs font-bold text-ink-900">👤 Nhân Sự Mới Nhận Việc</span>
                    </label>
                  </div>
                </div>
              </div>
        ...
        {activeTab === 'SECURITY_AUDIT' && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-line shadow-sm p-6 space-y-6">
              <h3 className="font-bold text-sm text-ink-900 flex items-center gap-2 border-b border-line pb-4">
                <ShieldAlert className="w-5 h-5 text-success-fg" /> Cấu Hình Giới Hạn Bảo Mật & Maintenance Mode
              </h3>

              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <label className="block text-xs font-bold text-ink-700 mb-1">Dung Lượng Tải File Tối Đa (MB)</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-xs font-bold text-ink-700 mb-1">Thời Gian Hết Hạn Session (Phút)</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-xs font-bold text-ink-700 mb-1">Số Lần Đăng Nhập Sai Tối Đa</label>
                  <input
                    type="number"
              ...
              <div className="p-4 bg-brand-50 text-brand-800 rounded-2xl border border-line space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-ink-400">Khi bật, hệ thống sẽ thông báo bảo trì tạm thời cho người dùng thông thường.</p>
                    </div>
                  </div>
                  ...
                  <label className="relative inline-flex items-center cursor-pointer">
                    ...
                    />
                    <div className="w-11 h-6 bg-line-soft peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-line-strong after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-warn-fg"></div>
                  </label>
                </div>

                {config.security.maintenance_mode && (
                  <div>
                    <label className="block text-[11px] font-semibold text-ink-400 mb-1">Thông Báo Bảo Trì Hiển Thị:</label>
                    <input
                      type="text"
                      ...
                      className="w-full p-2 bg-surface-subtle border border-line text-warn-fg rounded-xl text-xs font-semibold"
                    />
                  </div>
                )}
              </div>
            ...
            <div className="bg-white rounded-2xl border border-line shadow-sm overflow-hidden space-y-2">
              <div className="p-4 border-b border-line flex items-center justify-between">
                <h3 className="text-xs font-extrabold uppercase tracking-wider text-ink-900 flex items-center gap-2">
                  <History className="w-4 h-4 text-brand-600" /> Nhật Ký Thay Đổi Cấu Hình (Config Audit Log)
                </h3>
                <span className="text-xs text-ink-500 font-medium">
                  <strong className="text-ink-900">{config.audit_logs?.length || 0} Lần Cập Nhật</strong>
                </span>
              </div>
                ...
                <table className="w-full text-left border-collapse text-xs">
                  ...
                  <tbody className="divide-y divide-line-soft">
                    {(!config.audit_logs || config.audit_logs.length === 0) ? (
                      <tr>
                        <td colSpan={5} className="p-8 text-center text-ink-400 italic">
                      ...
                      config.audit_logs.map((log) => (
                        <tr key={log.id} className="hover:bg-surface-subtle transition-colors">
                          <td className="p-4 font-mono text-ink-500">{log.timestamp}</td>
                          <td className="p-4 font-bold text-ink-900">{log.section}</td>
                          <td className="p-4 font-semibold text-brand-800">{log.actor_name}</td>
                          <td className="p-4">
                            <span className="px-2 py-0.5 bg-brand-50 text-brand-800 rounded font-mono text-[10px] font-bold">
                              {log.action}
                            </span>
                          </td>
                          <td className="p-4 text-ink-700">{log.details}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
