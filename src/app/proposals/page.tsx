...
function EmployeePickerSelect({
  ...
  return ( <div className="space-y-2 bg-white p-3 rounded-xl border border-slate-200 shadow-sm"> {/* Criteria Filter Buttons */} <div className="flex flex-wrap items-center gap-1.5 text-[11px] font-medium pb-2 border-b"> <span className="text-slate-500 font-semibold mr-1"> Tiêu chí chọn NV từ HRM:</span> <button
          type="button"
          onClick={() => setFilterMode('ALL')}
          className={`px-2.5 py-1 rounded-lg transition-all ${
            filterMode === 'ALL' ? 'bg-slate-900 text-white shadow-xs' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
          }`}
        > 🌐 Tất Cả ({employees.length}) </button> <button
          type="button"
...
export default function ProposalsPage() {
  ...
  const handleSaveTemplateSubmit = (e: React.FormEvent) => {
    ...
    if (isCreatingNewTmpl) {
      ...
      showToast(` Đã tạo mới Mẫu Phiếu Phê Duyệt: ${newTmpl.title}`);
      ...
      setTemplates([...updated]);
      showToast(`✏ Đã cập nhật cấu hình Mẫu Phiếu: ${updatedTmpl.title}`);
    }
  ...
  const handleToggleTemplateActive = (id: string) => {
    ...
    setTemplates([...updated]);
    showToast(' Đã cập nhật trạng thái Bật / Tắt của Mẫu Phiếu!');
  };
  ...
  const handleDeleteTemplate = (id: string) => {
    if (confirm('Bạn có chắc chắn muốn xóa loại phiếu phê duyệt này khỏi hệ thống?')) {
      ...
      setTemplates([...updated]);
      showToast('🗑 Đã xóa Mẫu Phiếu Phê Duyệt khỏi hệ thống!');
    }
  };
  ...
  return ( <div className="space-y-6"> {/* Toast Notification */}
      ...
      {/* Header - Clean White with Colorful Highlights */} <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-5 sm:p-6 rounded-xl border border-slate-200/80 dark:border-slate-800 shadow-sm"> <div className="flex items-center gap-3"> <div className="w-10 h-10 rounded-lg bg-purple-50 dark:bg-purple-950/50 border border-purple-200 dark:border-purple-900 flex items-center justify-center text-purple-600 dark:text-purple-400"> <ShieldCheck className="w-5 h-5" /> </div> <div> <div className="flex items-center gap-2"> <h1 className="text-xl sm:text-2xl font-semibold tracking-tight text-slate-900 dark:text-slate-100"> Báo Giá & Phê Duyệt Đề Xuất </h1> <span className="px-2.5 py-0.5 rounded-md bg-purple-50 dark:bg-purple-950/60 text-purple-700 dark:text-purple-300 text-[11px] font-medium border border-purple-200 dark:border-purple-800"> {templates.length} Mẫu Phiếu </span> </div> <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5"> Hệ thống quản lý phê duyệt đa cấp tích hợp HRM & báo giá dịch vụ </p> </div> </div> <div className="flex items-center gap-2"> <button
        ...
        {activeTab === 'SUBMISSIONS' && ( <div className="space-y-4"> <div className="flex flex-col sm:flex-row items-center justify-between gap-3"> <div className="flex items-center gap-2 w-full sm:w-auto"> <div className="relative w-full sm:w-80"> <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /> <input
                ...
                > <option value="ALL">Tất Cả Trạng Thái</option> <option value="PENDING">⏳ Đang Trình Duyệt</option> <option value="APPROVED"> Đã Phê Duyệt</option> <option value="REJECTED"> Từ Chối</option> </select> </div> <span className="text-slate-500"> Hiển thị <strong className="text-slate-900">{filteredSubmissions.length}</strong> phiếu phê duyệt </span> </div> <div className="overflow-x-auto border border-slate-200 rounded-xl"> <table className="w-full text-left border-collapse"> <thead> <tr className="bg-slate-100/80 border-b border-slate-200 text-slate-700 font-semibold uppercase text-[10.5px]"> <th className="p-3">Mã & Loại Phiếu Phê Duyệt</th> <th className="p-3">Người Nộp & Phòng Ban</th> <th className="p-3">Ngày Trình Ký</th> <th className="p-3 text-center">Tiến Độ Các Bước Duyệt</th> <th className="p-3 text-center">Trạng Thái</th> <th className="p-3 text-center">Thao Tác</th> </tr> </thead> <tbody className="divide-y divide-slate-100 font-medium"> {filteredSubmissions.map((sub) => ( <tr key={sub.id} className="hover:bg-slate-50 transition-colors"> <td className="p-3"> <p className="font-semibold text-slate-900 text-sm leading-snug">{sub.template_title}</p> <p className="font-mono text-purple-700 text-[11px] font-medium">Mã số: {sub.proposal_code}</p> </td> <td className="p-3"> <p className="font-medium text-slate-800">{sub.applicant_name}</p> <p className="text-[11px] text-slate-500">🏢 {sub.applicant_department}</p> </td> <td className="p-3 font-mono font-medium text-slate-600">📅 {formatDate(sub.submitted_date)}</td> <td className="p-3 text-center"> <div className="flex items-center justify-center gap-1 font-mono text-[10.5px]"> {sub.approval_steps.map((st) => ( <span
                              key={st.step_order}
                            ...
                            > {st.status === 'APPROVED' ? '✓' : st.status === 'REJECTED' ? '✕' : st.step_order} </span> ))} </div> </td> <td className="p-3 text-center"> <span className={`px-2.5 py-1 rounded-full font-semibold text-[10.5px] ${
                        ...
                        }`}> {sub.status === 'PENDING' ? '⏳ Đang Trình Duyệt' : sub.status === 'APPROVED' ? ' Đã Phê Duyệt' : ' Từ Chối'} </span> </td> <td className="p-3 text-center"> <button
                          onClick={() => {
                            setSelectedSub(sub);
                            setIsViewSubOpen(true);
                          }}
                          className="px-3 py-1.5 bg-purple-50 text-purple-700 hover:bg-purple-100 rounded-xl font-semibold flex items-center gap-1 mx-auto transition-all"
                        > <Eye className="w-3.5 h-3.5" /> Xem Form & Duyệt </button> </td> </tr> ))} </tbody> </table> </div> </div> )}

        {/* TAB 2: 2-STEP FORM CREATION (STEP 1: SELECT TEMPLATE CARD -> STEP 2: DEDICATED FORM SCREEN) */}
        {activeTab === 'CREATE_NEW' && ( <div className="space-y-6"> {/* STEP 1: SELECT TEMPLATE CARD SCREEN */}
            {createFormStep === 'SELECT_TEMPLATE' && ( <div className="p-6 bg-slate-50 border border-slate-200 rounded-xl space-y-6 text-xs font-medium animate-in fade-in duration-200"> <div className="border-b pb-3 flex items-center justify-between"> <div> <h3 className="font-semibold text-sm text-slate-900">Bước 1: Chọn Loại Phiếu Phê Duyệt</h3> <p className="text-[11px] text-slate-500 font-normal mt-0.5">Vui lòng bấm vào một mẫu phiếu dưới đây để chuyển sang màn hình điền thông tin chi tiết.</p> </div> <span className="px-3 py-1 bg-purple-100 text-purple-900 rounded-full text-[11px] font-semibold"> Bước 1 / 2 </span> </div> {/* CATEGORY GROUP TABS */} <div className="flex items-center gap-2 overflow-x-auto pb-2"> {groupedActiveTemplates.map((grp) => ( <button
                      key={grp.category}
                      type="button"
                    ...
                    > {grp.category === 'Hành Chính & Nhân Sự' && <Users className="w-4 h-4 text-blue-300" />}
                      ...
                      {grp.category} ({grp.templates.length}) </button> ))} </div> {/* CARD GRID OF TEMPLATES IN SELECTED CATEGORY GROUP */} <div className="space-y-3"> <h4 className="font-semibold text-slate-900 text-xs text-purple-700 uppercase tracking-wider flex items-center gap-1.5"> <FolderTree className="w-4 h-4" /> Danh Sách Các Loại Phiếu Thuộc Nhóm: {selectedCategoryGroupTab} </h4> <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4"> {groupedActiveTemplates
                      ...
                      ?.templates.map((tmpl) => ( <div
                          ...
                          className="p-5 rounded-xl border cursor-pointer transition-all space-y-3 bg-white border-slate-200 hover:border-purple-500 hover:shadow-xl hover:-translate-y-1 group relative flex flex-col justify-between"
                        > <div className="space-y-2"> <div className="flex items-center justify-between"> <span className="px-2.5 py-0.5 rounded-full bg-purple-50 text-purple-700 font-mono text-[10.5px] font-semibold border border-purple-200"> {tmpl.template_code} </span> <span className="text-purple-600 opacity-0 group-hover:opacity-100 transition-opacity font-medium text-xs flex items-center gap-0.5"> Chọn điền form <ArrowRight className="w-3.5 h-3.5" /> </span> </div> <h5 className="font-semibold text-slate-900 text-sm leading-snug group-hover:text-purple-700 transition-colors">{tmpl.title}</h5> <p className="text-[11px] text-slate-500 font-normal line-clamp-3 leading-relaxed">{tmpl.description}</p> </div> <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-[10.5px] text-slate-600 font-mono font-medium"> <span>📄 {tmpl.fields.length} Trường</span> <span className="text-purple-700">🔄 {tmpl.approval_steps.length} Cấp duyệt</span> </div> </div> ))} </div> </div> </div> )}

            {/* STEP 2: DEDICATED FORM FILLING SCREEN */}
            {createFormStep === 'FILL_FORM' && currentTemplate && ( <div className="space-y-4 animate-in fade-in zoom-in-95 duration-200"> {/* NAVIGATION BACK BAR */} <div className="flex items-center justify-between bg-white p-4 rounded-xl border border-purple-200 shadow-sm"> <button
                    type="button"
                    onClick={() => setCreateFormStep('SELECT_TEMPLATE')}
                    className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 rounded-xl text-xs font-semibold flex items-center gap-2 transition-all active:scale-95"
                  > <ArrowLeft className="w-4 h-4 text-purple-600" /> Quay Lại Chọn Loại Phiếu Khác </button> <span className="px-3 py-1 bg-purple-100 text-purple-900 rounded-full text-xs font-semibold"> Bước 2 / 2: Điền Thông Tin Phiếu Phê Duyệt </span> </div> {/* FORM CONTAINER */} <form onSubmit={handleCreateSubmissionSubmit} className="space-y-6 max-w-3xl mx-auto bg-white p-8 rounded-xl border border-purple-300 shadow-2xl"> <div className="border-b border-purple-100 pb-4 flex items-start justify-between"> <div> <div className="flex items-center gap-2"> <span className="px-3 py-0.5 rounded-full bg-purple-100 text-purple-900 font-mono text-xs font-semibold"> {currentTemplate.template_code} </span> <span className="px-3 py-0.5 bg-amber-50 text-amber-900 border border-amber-200 rounded-full font-medium text-xs"> {currentTemplate.category_name} </span> </div> <h2 className="font-semibold text-lg text-slate-900 mt-2">{currentTemplate.title}</h2> <p className="text-xs text-slate-500 font-medium mt-1">{currentTemplate.description}</p> </div> </div> {/* APPROVAL STEPS FLOW PREVIEW */} <div className="p-4 bg-purple-50/70 border border-purple-200/80 rounded-xl text-purple-900 font-medium text-xs space-y-1.5"> <p className="text-purple-950 font-semibold">🔄 Luồng Ký Phê Duyệt Đa Cấp ({currentTemplate.approval_steps.length} cấp):</p> <div className="flex flex-wrap items-center gap-2 font-mono text-purple-800 text-[11px]"> {currentTemplate.approval_steps.map((st, idx) => ( <React.Fragment key={st.step_order}> <span className="px-2.5 py-1 bg-white border border-purple-300 rounded-lg shadow-2xs"> Cấp {st.step_order}: {st.approver_role} </span> {idx < currentTemplate.approval_steps.length - 1 && <span className="text-purple-400">➔</span>} </React.Fragment> ))} </div> </div> {/* RENDER FORM FIELDS WITH EMPLOYEE_SELECT INTEGRATION */} <div className="space-y-5 pt-2"> <h3 className="font-semibold text-sm text-slate-900 border-b pb-2">📋 Điền Thông Tin Dữ Liệu Trường</h3> {currentTemplate.fields.map((f) => ( <div key={f.id} className="space-y-1.5 bg-slate-50/80 p-4 rounded-xl border border-slate-200"> <label className="block text-slate-800 font-medium text-xs"> {f.field_label} {f.is_required && <span className="text-red-500">*</span>} <span className="text-[10px] text-purple-600 font-mono ml-2 font-semibold">[{f.data_type}]</span> </label> {/* EMPLOYEE SELECT COMPONENT */}
                        {f.data_type === 'EMPLOYEE_SELECT' && ( <EmployeePickerSelect
                        ...
                        {f.data_type === 'TEXT_AREA' && ( <textarea
                            rows={3}
                            required={f.is_required}
                        ...
                        {f.data_type === 'NUMBER_AMOUNT' && ( <input
                            type="number"
                            required={f.is_required}
                            placeholder={f.placeholder || '0'}
                            value={formDataValues[f.field_name] || ''}
                            onChange={(e) => handleFieldInputChange(f.field_name, Number(e.target.value))}
                            className="w-full px-3.5 py-2.5 bg-white border rounded-xl font-mono text-purple-700 font-medium text-sm shadow-2xs"
                          /> )}

                        {f.data_type === 'DATE_PICKER' && ( <input
                            type="date"
                            required={f.is_required}
                            value={formDataValues[f.field_name] || ''}
                            onChange={(e) => handleFieldInputChange(f.field_name, e.target.value)}
                        ...
                        {f.data_type === 'SELECT_DROPDOWN' && ( <select
                            required={f.is_required}
                            value={formDataValues[f.field_name] || f.options?.[0] || ''}
                            onChange={(e) => handleFieldInputChange(f.field_name, e.target.value)}
                            className="w-full px-3.5 py-2.5 bg-white border rounded-xl font-medium text-slate-800 text-xs shadow-2xs"
                          > <option value="">-- Chọn tùy chọn --</option> {f.options?.map((opt, idx) => ( <option key={idx} value={opt}>{opt}</option> ))} </select> )}

                        {f.data_type === 'FILE_UPLOAD' && ( <input
                            type="file"
                            required={f.is_required}
                        ...
                        {f.data_type === 'CHECKBOX_BOOLEAN' && ( <div className="flex items-center gap-2 pt-1"> <input
                              type="checkbox"
                              checked={!!formDataValues[f.field_name]}
                              onChange={(e) => handleFieldInputChange(f.field_name, e.target.checked)}
                              className="w-4 h-4 accent-purple-600 rounded"
                            /> <span className="text-slate-700 font-medium text-xs">Đồng ý & Xác nhận điều khoản</span> </div> )} </div> ))} </div> <div className="flex items-center justify-between pt-6 border-t border-slate-200"> <button
                      type="button"
                      onClick={() => setCreateFormStep('SELECT_TEMPLATE')}
                      className="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-medium flex items-center gap-1.5 text-xs transition-all"
                    > <ArrowLeft className="w-4 h-4" /> Quay Lại </button> <button
                      type="submit"
                      className="px-7 py-3 bg-purple-600 hover:bg-purple-700 text-white font-semibold rounded-xl shadow-xl shadow-purple-600/30 flex items-center gap-2 text-xs transition-all active:scale-95"
                    > <FileCheck className="w-4 h-4" /> Nộp Phiếu Phê Duyệt Này </button> </div> </form> </div> )} </div> )}

        {/* TAB 3: ADMIN TEMPLATE CONFIGURATOR (XEM, SỬA, XÓA, BẬT/TẮT CÁC LOẠI PHIẾU) */}
        {activeTab === 'TEMPLATE_CONFIG' && ( <div className="p-6 bg-slate-50 border border-slate-200 rounded-xl space-y-6 text-xs font-medium"> <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b pb-3"> <div> <h3 className="font-semibold text-sm text-slate-900 flex items-center gap-2"> <Settings className="w-5 h-5 text-amber-600" /> Trình Cấu Hình Loại Phiếu Phê Duyệt (Template Manager) </h3> <p className="text-[11px] text-slate-500 font-normal mt-0.5"> Quản lý 22 mẫu phiếu phê duyệt chuẩn Lark Approval. Hỗ trợ Admin: <strong>Xem cấu hình</strong>, <strong>Chỉnh sửa trường & luồng duyệt</strong>, <strong>Xóa</strong>, <strong>Bật/Tắt active</strong> và <strong>Tạo mẫu phiếu mới</strong>. </p> </div> <button
                onClick={handleOpenCreateTemplateModal}
                className="px-4 py-2.5 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-semibold shadow-md flex items-center gap-1.5 shrink-0"
              > <Plus className="w-4 h-4 text-amber-400" /> ➕ Tạo Mẫu Phiếu Mới </button> </div> {/* FILTER BAR FOR TEMPLATES */} <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-white p-3 rounded-xl border border-slate-200"> <div className="flex items-center gap-2 w-full sm:w-auto"> <div className="relative w-full sm:w-72"> <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /> <input
                    type="text"
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                  ...
                  className="px-3 py-1.5 bg-slate-50 border rounded-xl font-medium text-slate-700 text-xs"
                > {CATEGORIES.map((cat, idx) => ( <option key={idx} value={cat}>{cat}</option> ))} </select> <select
                  value={selectedStatusFilter}
                  onChange={(e) => setSelectedStatusFilter(e.target.value as any)}
                  className="px-3 py-1.5 bg-slate-50 border rounded-xl font-medium text-slate-700 text-xs"
                > <option value="ALL">Tất Cả Trạng Thái</option> <option value="ACTIVE">🟢 Đang Bật (Active)</option> <option value="INACTIVE">⚪ Đang Tắt (Inactive)</option> </select> </div> <span className="text-slate-500 font-semibold shrink-0"> Hiển thị <strong className="text-purple-700">{filteredTemplates.length}</strong> / 22 Mẫu Phiếu </span> </div> {/* TEMPLATES TABLE WITH FULL ACTION BUTTONS (XEM, SỬA, XÓA, BẬT/TẮT) */} <div className="overflow-x-auto border border-slate-200 rounded-xl bg-white shadow-sm"> <table className="w-full text-left border-collapse"> <thead> <tr className="bg-slate-100/80 border-b border-slate-200 text-slate-700 font-semibold uppercase text-[10.5px]"> <th className="p-3">Mã & Tên Mẫu Phiếu</th> <th className="p-3">Nhóm Danh Mục</th> <th className="p-3 text-center">Số Trường Form</th> <th className="p-3 text-center">Số Cấp Duyệt</th> <th className="p-3 text-center">Trạng Thái (Bật/Tắt)</th> <th className="p-3 text-center">Thao Tác Cấu Hình</th> </tr> </thead> <tbody className="divide-y divide-slate-100 font-medium"> {filteredTemplates.map((t) => ( <tr key={t.id} className={`hover:bg-slate-50 transition-colors ${!t.is_active ? 'opacity-60 bg-slate-50/50' : ''}`}> <td className="p-3"> <p className="font-semibold text-slate-900 text-xs leading-snug">{t.title}</p> <p className="font-mono text-purple-700 text-[11px] font-medium">Mã phiếu: {t.template_code}</p> <p className="text-[11px] text-slate-500 truncate max-w-xs">{t.description}</p> </td> <td className="p-3"> <span className="px-2.5 py-1 bg-amber-50 text-amber-800 border border-amber-200 rounded-full font-medium text-[10.5px] inline-block"> {t.category_name} </span> </td> <td className="p-3 text-center font-mono font-medium text-slate-700"> {t.fields.length} Trường </td> <td className="p-3 text-center font-mono text-[10.5px] text-purple-700 font-medium"> {t.approval_steps.length} Cấp Duyệt </td> <td className="p-3 text-center"> <button
                          onClick={() => handleToggleTemplateActive(t.id)}
                          className={`px-3 py-1 rounded-full font-semibold text-[10.5px] border transition-all flex items-center gap-1.5 mx-auto ${
                        ...
                        > {t.is_active ? <ToggleRight className="w-4 h-4 text-emerald-600" /> : <ToggleLeft className="w-4 h-4 text-slate-400" />}
                          {t.is_active ? '🟢 Đang Bật' : '⚪ Đang Tắt'} </button> </td> <td className="p-3 text-center"> <div className="flex items-center justify-center gap-1.5"> {/* VIEW PREVIEW SCHEMA */} <button
                            onClick={() => {
                              setPreviewTemplate(t);
                              setIsPreviewTmplOpen(true);
                            }}
                            className="px-2.5 py-1 bg-purple-50 hover:bg-purple-100 text-purple-700 rounded-lg text-[11px] font-medium flex items-center gap-1"
                            title="Xem Cấu Hình Mẫu Form & Luồng Duyệt"
                          > <Eye className="w-3.5 h-3.5" /> Xem </button> {/* EDIT TEMPLATE */} <button
                            onClick={() => handleOpenEditTemplateModal(t)}
                            className="px-2.5 py-1 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg text-[11px] font-medium flex items-center gap-1"
                            title="Sửa Mẫu Phiếu, Trường & Luồng Duyệt"
                          > <Edit className="w-3.5 h-3.5" /> Sửa </button> {/* DUPLICATE */} <button
                            onClick={() => handleDuplicateTemplate(t.id)}
                            className="p-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg"
                            title="Nhân bản Mẫu Form 1-Click"
                          > <Copy className="w-3.5 h-3.5" /> </button> {/* DELETE */} <button
      ...
      {isEditTmplOpen && ( <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in zoom-in duration-200"> <div className="bg-white rounded-xl border border-slate-200 shadow-2xl w-full max-w-3xl overflow-hidden p-6 space-y-4 text-xs font-medium max-h-[90vh] overflow-y-auto"> <div className="flex items-center justify-between border-b pb-3"> <div> <h3 className="font-semibold text-sm text-slate-900 flex items-center gap-2"> <Edit className="w-4 h-4 text-purple-600" /> {isCreatingNewTmpl ? 'Tạo Mới Loại Phiếu Phê Duyệt' : `Chỉnh Sửa Mẫu Phiếu: ${editTmplTitle}`} </h3> </div> <button onClick={() => setIsEditTmplOpen(false)} className="p-1 rounded-lg text-slate-400 hover:text-slate-700"> <X className="w-5 h-5" /> </button> </div> <form onSubmit={handleSaveTemplateSubmit} className="space-y-4"> <div className="grid grid-cols-1 sm:grid-cols-3 gap-3"> <div> <label className="block text-slate-700 mb-1">Mã Mẫu Biểu *</label> <input
                    type="text"
                    required
                    value={editTmplCode}
                  ...
                  /> </div> <div className="sm:col-span-2"> <label className="block text-slate-700 mb-1">Tên Mẫu Biểu Phê Duyệt *</label> <input
                    type="text"
                    required
                    value={editTmplTitle}
                    onChange={(e) => setEditTmplTitle(e.target.value)}
                    className="w-full px-3 py-2 border rounded-xl font-semibold"
                  /> </div> </div> <div className="grid grid-cols-1 sm:grid-cols-2 gap-3"> <div> <label className="block text-slate-700 mb-1">Nhóm Danh Mục *</label> <select
                  ...
                  > {CATEGORIES.filter(c => c !== 'Tất Cả Danh Mục').map((cat, idx) => ( <option key={idx} value={cat}>{cat}</option> ))} </select> </div> <div> <label className="block text-slate-700 mb-1">Mô Tả Quy Định Mẫu Biểu</label> <input
                    type="text"
                    value={editTmplDesc}
                    onChange={(e) => setEditTmplDesc(e.target.value)}
                    className="w-full px-3 py-2 border rounded-xl"
                  /> </div> </div> {/* DYNAMIC FIELDS EDITOR */} <div className="pt-3 border-t space-y-3"> <div className="flex items-center justify-between"> <h4 className="font-semibold text-purple-700 text-xs uppercase tracking-wider"> 📄 Cấu Hình Danh Sách Trường Thông Tin ({editTmplFields.length} Trường) </h4> <button
                    type="button"
                    onClick={handleAddFieldInEditModal}
                    className="px-3 py-1.5 bg-purple-50 text-purple-700 hover:bg-purple-100 rounded-xl font-semibold flex items-center gap-1 transition-all"
                  > <Plus className="w-3.5 h-3.5" /> Thêm Trường Mới </button> </div> <div className="space-y-2 max-h-60 overflow-y-auto pr-1"> {editTmplFields.map((f, idx) => ( <div key={f.id} className="p-3 bg-slate-50 border border-slate-200 rounded-xl grid grid-cols-1 sm:grid-cols-4 gap-2 items-center"> <div> <label className="text-[10px] text-slate-500 block">Tên Nhãn Trường #{idx + 1}</label> <input
                          type="text"
                          value={f.field_label}
                          onChange={(e) => {
                        ...
                        /> </div> <div> <label className="text-[10px] text-slate-500 block">Kiểu Dữ Liệu Trường *</label> <select
                          value={f.data_type}
                          onChange={(e) => {
                            const updated = [...editTmplFields];
                        ...
                        > <option value="TEXT_INPUT">TEXT_INPUT (Chữ ngắn)</option> <option value="TEXT_AREA">TEXT_AREA (Diễn giải)</option> <option value="NUMBER_AMOUNT">NUMBER_AMOUNT (Số tiền/Số)</option> <option value="DATE_PICKER">DATE_PICKER (Chọn ngày)</option> <option value="SELECT_DROPDOWN">SELECT_DROPDOWN (Danh sách)</option> <option value="FILE_UPLOAD">FILE_UPLOAD (Tệp đính kèm)</option> <option value="CHECKBOX_BOOLEAN">CHECKBOX_BOOLEAN (Hộp chọn)</option> <option value="EMPLOYEE_SELECT">👤 EMPLOYEE_SELECT (Chọn Nhân Viên HRM)</option> </select> </div> <div className="flex items-center gap-2 pt-3"> <input
                          type="checkbox"
                          ...
                          className="w-4 h-4 accent-purple-600 rounded"
                        /> <span className="text-slate-700 font-medium text-[11px]">Bắt buộc</span> </div> <div className="text-right pt-3"> <button
                          type="button"
                          onClick={() => setEditTmplFields(editTmplFields.filter((_, i) => i !== idx))}
                          className="p-1 bg-red-50 text-red-600 rounded hover:bg-red-100"
                        > <Trash2 className="w-3.5 h-3.5" /> </button> </div> </div> ))} </div> </div> {/* APPROVAL STEPS EDITOR */} <div className="pt-3 border-t space-y-3"> <div className="flex items-center justify-between"> <h4 className="font-semibold text-amber-700 text-xs uppercase tracking-wider"> 🔄 Cấu Hình Luồng Duyệt Đa Cấp ({editTmplSteps.length} Cấp) </h4> <button
                    type="button"
                    onClick={handleAddStepInEditModal}
                    className="px-3 py-1.5 bg-amber-50 text-amber-800 hover:bg-amber-100 rounded-xl font-semibold flex items-center gap-1 transition-all"
                  > <Plus className="w-3.5 h-3.5" /> Thêm Cấp Duyệt </button> </div> <div className="space-y-2"> {editTmplSteps.map((st, idx) => ( <div key={idx} className="p-2.5 bg-amber-50/50 border border-amber-200 rounded-xl flex items-center gap-3"> <span className="font-mono font-medium text-amber-900">Bước {idx + 1}:</span> <input
                        type="text"
                        ...
                        className="flex-1 px-3 py-1 border rounded-lg bg-white font-medium"
                        placeholder="Vai trò duyệt (Ví dụ: Kế Toán Trưởng)..."
                      /> <button
                        type="button"
                        onClick={() => setEditTmplSteps(editTmplSteps.filter((_, i) => i !== idx))}
                        className="p-1 text-red-600 hover:bg-red-50 rounded"
                      > <X className="w-4 h-4" /> </button> </div> ))} </div> </div> <div className="flex items-center justify-end gap-3 pt-4 border-t"> <button
                  type="button"
                  onClick={() => setIsEditTmplOpen(false)}
                  className="px-4 py-2 bg-slate-100 text-slate-700 rounded-xl font-medium"
                > Hủy </button> <button
                  type="submit"
                  className="px-5 py-2.5 bg-slate-900 hover:bg-slate-800 text-white font-semibold rounded-xl shadow-lg flex items-center gap-1.5"
                > <Save className="w-4 h-4 text-amber-400" /> Lưu Cấu Hình Mẫu Phiếu </button> </div> </form> </div> </div> )}

      {/* MODAL 3: XEM CHI TIẾT PHIẾU ĐÃ NỘP & THAO TÁC PHÊ DUYỆT */}
      {isViewSubOpen && selectedSub && ( <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in zoom-in duration-200"> <div className="bg-white rounded-xl border border-slate-200 shadow-2xl w-full max-w-2xl overflow-hidden p-6 space-y-4 text-xs font-medium max-h-[90vh] overflow-y-auto"> <div className="flex items-center justify-between border-b pb-3"> <div> <span className="font-mono text-xs font-semibold text-purple-700">{selectedSub.proposal_code}</span> <h3 className="font-semibold text-sm text-slate-900">Chi Tiết Phiếu Phê Duyệt & Ký Duyệt</h3> </div> <button onClick={() => setIsViewSubOpen(false)} className="p-1 rounded-lg text-slate-400 hover:text-slate-700"> <X className="w-5 h-5" /> </button> </div> <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-2"> <h4 className="font-semibold text-slate-900 text-sm">{selectedSub.template_title}</h4> <div className="grid grid-cols-2 gap-2 text-slate-600 font-medium pt-2 border-t border-slate-200/80"> <p>Người làm đơn: <strong className="text-slate-900">{selectedSub.applicant_name}</strong></p> <p>Đơn vị / Phòng ban: <strong className="text-slate-900">{selectedSub.applicant_department}</strong></p> <p>Ngày nộp phiếu: <strong className="text-slate-900 font-mono">{selectedSub.submitted_date}</strong></p> <p>Trạng thái: <strong className="text-purple-700">{selectedSub.status}</strong></p> </div> </div> {/* DISPLAY ALL FILLED FORM DATA FIELDS */} <div className="p-4 bg-purple-50/60 border border-purple-200 rounded-xl space-y-3"> <h4 className="font-semibold text-purple-900 text-xs uppercase tracking-wider"> 📄 Dữ Liệu Chi Tiết Đã Điền Trong Form Phiếu Phê Duyệt: </h4> <div className="space-y-2 bg-white p-4 rounded-xl border border-purple-100 font-medium"> {Object.entries(selectedSub.field_values).map(([key, val]) => ( <div key={key} className="flex flex-col sm:flex-row justify-between border-b border-slate-100 pb-1.5"> <span className="text-slate-500 font-medium">{key}:</span> <span className="text-slate-900 font-semibold font-mono"> {typeof val === 'number' ? formatCurrency(val) : String(val)} </span> </div> ))} </div> </div> {/* APPROVAL STEPS PROGRESSION */} <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-2"> <h4 className="font-semibold text-slate-900 text-xs uppercase tracking-wider"> 🔄 Tiến Độ Luồng Duyệt ({selectedSub.approval_steps.length} Cấp): </h4> <div className="space-y-1.5"> {selectedSub.approval_steps.map((st) => ( <div key={st.step_order} className="p-2.5 bg-white border border-slate-200 rounded-xl flex items-center justify-between"> <div> <p className="font-medium text-slate-900">Bước {st.step_order}: {st.approver_role}</p> {st.approver_name && <p className="text-[11px] text-slate-500">👤 {st.approver_name} • {st.approved_at}</p>}
                      {st.comment && <p className="text-[11px] text-emerald-700 italic"> "{st.comment}"</p>} </div> <span className={`px-2.5 py-0.5 rounded font-semibold text-[10.5px] ${
                      st.status === 'APPROVED' ? 'bg-emerald-100 text-emerald-800' :
                      st.status === 'REJECTED' ? 'bg-red-100 text-red-800' : 'bg-amber-100 text-amber-800'
                    }`}> {st.status === 'APPROVED' ? '✓ Đã Duyệt' : st.status === 'REJECTED' ? '✕ Từ Chối' : '⏳ Chờ Duyệt'} </span> </div> ))} </div> </div> {/* ACTION SECTION FOR APPROVER */}
            {selectedSub.status === 'PENDING' && ( <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl space-y-3"> <h4 className="font-semibold text-amber-900 text-xs"> Ký Phê Duyệt Hoặc Từ Chối Phiếu:</h4> <div className="space-y-2"> <input
                    type="text"
                  ...
                  /> <div className="flex gap-2"> <button
                      ...
                      className="flex-1 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-xl shadow-md flex items-center justify-center gap-1"
                    > <CheckCircle2 className="w-4 h-4" /> Phê Duyệt Cấp {selectedSub.current_step_order} </button> <button
                      onClick={handleRejectProposal}
                      className="px-4 py-2.5 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-xl shadow-md flex items-center justify-center gap-1"
                    > <X className="w-4 h-4" /> Từ Chối </button> </div> <input
                    type="text"
                  ...
                  /> </div> </div> )} <div className="flex items-center justify-end pt-2"> <button
