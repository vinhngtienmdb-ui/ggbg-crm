...
export default function EnterpriseProjectsPage() {
  ...
  const handleDelete = (id: string) => {
    ...
    setProjects([...updated]);
    showToast('🗑️ Đã xóa dự án khỏi hệ thống');
  };
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold text-slate-900">Quản Lý Dự Án Enterprise & Sơ Đồ Tiến Độ (PSA)</h1>
            <span className="px-2.5 py-0.5 rounded-full bg-purple-50 text-purple-700 text-xs font-bold border border-purple-200">
              Module ERP Dự Án
            </span>
          </div>
          <p className="text-xs text-slate-500 mt-1">
            Theo dõi tiến độ dự án theo sơ đồ Gantt Chart, quản lý mốc thời gian Milestone & so sánh ngân sách dự kiến vs chi phí thực tế.
          </p>
        </div>
          ...
          <div className="bg-slate-100 p-1 rounded-xl flex items-center gap-1 text-xs font-bold">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg transition-all ${
                viewMode === 'GANTT' ? 'bg-white text-purple-700 shadow-sm' : 'text-slate-600 hover:text-slate-900'
              }`}
            >
              📊 Sơ Đồ Gantt Chart
            </button>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm space-y-6 text-xs font-bold">
          ...
          <div className="space-y-6">
            {filteredProjects.map((prj) => (
              <div key={prj.id} className="p-5 bg-slate-50 border border-slate-200 rounded-2xl space-y-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-semibold ${
                        ...
                        {prj.status === 'IN_PROGRESS' ? '🔵 Đang Thực Hiện' : prj.status === 'PLANNING' ? '🟡 Lập Kế Hoạch' : '🟢 Hoàn Thành'}
                      </span>
                    </div>
                    <p className="text-[11px] text-slate-500 font-normal mt-0.5">
                      Khách hàng: <strong>{prj.client_name || 'Internal'}</strong> • Trưởng dự án: <strong>{prj.manager_name}</strong> ({prj.department})
                    </p>
                  </div>
                ...
                {prj.tasks.length > 0 && (
                  <div className="pt-2 space-y-2">
                    <span className="text-[11px] text-slate-500 font-bold uppercase tracking-wider block">Các Hạng Mục Công Việc trực thuộc ({prj.tasks.length} Task):</span>
                      ...
                      {prj.tasks.map((t) => (
                        <div key={t.id} className="p-2.5 bg-white border border-slate-200 rounded-xl flex items-center justify-between text-xs">
                          ...
                          <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                            ...
                            {t.status === 'DONE' ? '✅ Finished' : t.status === 'IN_PROGRESS' ? '🔵 Doing' : '⚪ Todo'}
                          </span>
                        </div>
                      ))}
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in zoom-in duration-200">
          <div className="bg-white rounded-xl border border-slate-200 shadow-2xl w-full max-w-lg overflow-hidden p-6 space-y-4 text-xs font-bold">
            ...
            <form onSubmit={handleCreateSubmit} className="space-y-3">
              <div>
                <label className="block text-slate-700 mb-1">Tên Dự Án Enterprise *</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-700 mb-1">Tên Khách Hàng / Đối Tác</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-slate-700 mb-1">Quản Lý Dự Án (PM) *</label>
                  <input
                    type="text"
                    required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-700 mb-1">Ngày Bắt Đầu *</label>
                  <input
                    type="date"
                    required
                ...
                <div>
                  <label className="block text-slate-700 mb-1">Ngày Dự Kiến Kết Thúc *</label>
                  <input
                    type="date"
                    required
              ...
              <div>
                <label className="block text-slate-700 mb-1">Tổng Ngân Sách Dự Kiến (VND) *</label>
                <input
                  type="number"
                  step={5000000}
                  required
              ...
              <div>
                <label className="block text-slate-700 mb-1">Mô Tả Mục Tiêu Dự Án</label>
                <textarea
                  rows={3}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in zoom-in duration-200">
          <div className="bg-white rounded-xl border border-slate-200 shadow-2xl w-full max-w-md overflow-hidden p-6 space-y-4 text-xs font-bold">
            ...
            <form onSubmit={handleAddTaskSubmit} className="space-y-3">
              <div>
                <label className="block text-slate-700 mb-1">Tên Hạng Mục Công Việc *</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-slate-700 mb-1">Nhân Sự Phụ Trách *</label>
                <input
                  type="text"
                  required
