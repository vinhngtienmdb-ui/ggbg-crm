...
import { formatNumber, formatNumberWithUnit } from '@/lib/formatters';
...
export default function UnifiedKpisPerformancePage() {
  ...
  const handleDeleteKpi = (id: string) => {
    if (confirm('Bạn có chắc chắn muốn xóa chỉ tiêu KPI này không?')) {
      deleteKPI(id);
      showToast('🗑 Đã xóa chỉ tiêu KPI thành công!');
    }
  };
  ...
  const handleAutoOpenPeriod = () => {
    ...
    if (createdScorecards.length > 0) {
      ...
      showToast(`ℹ Đợt chấm điểm ${selectedPeriod} đã được khởi tạo trước đó.`);
    }
    reloadAllData();
  };
  ...
  const handleTransitionPeriod = (target: 'MANAGER' | 'HR' | 'FINAL') => {
    if (target === 'MANAGER') {
      ...
      showToast(` Đã gửi toàn bộ phiếu chấm điểm sang Bộ Phận Nhân Sự (HR) thẩm định!`);
  ...
  const handleDeleteScorecard = (id: string) => {
    if (confirm('Bạn có chắc chắn muốn xóa phiếu chấm điểm này không?')) {
      deleteScorecard(id);
      showToast('🗑 Đã xóa phiếu chấm điểm hiệu suất!');
      reloadAllData();
    }
  };
  ...
  const handleSaveWeights = (newWeights: FormulaWeights) => {
    ...
    showToast(' Đã lưu công thức tính điểm 3P thành công!');
  ...
  return ( <div className="space-y-6"> {/* Toast Notification */}
      ...
      {activeMainTab === 'KPI_LIST' && ( <div className="space-y-4 text-xs font-medium"> {/* Filters Bar */} <div className="bg-white p-4 rounded-xl border border-slate-200/80 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4"> <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto"> <div className="relative w-full sm:w-72"> <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /> <input
                ...
                className="w-full sm:w-auto px-3 py-2 bg-slate-50 border rounded-xl font-medium"
              > <option value="ALL">Tất Cả Cấp Độ</option> <option value="COMPANY">Cấp Công Ty</option> <option value="DEPARTMENT">Cấp Phòng Ban</option> <option value="INDIVIDUAL">Cấp Cá Nhân</option> </select> <select
                value={selectedDeptFilter}
                onChange={(e) => setSelectedDeptFilter(e.target.value)}
                className="w-full sm:w-auto px-3 py-2 bg-slate-50 border rounded-xl font-medium"
              > <option value="ALL">Tất Cả Phòng Ban</option> <option value="Phòng Kinh Doanh 1">Phòng Kinh Doanh 1</option> <option value="Phòng Vận Hành TMĐT">Phòng Vận Hành TMĐT</option> <option value="Phòng CSKH">Phòng CSKH</option> <option value="Phòng Marketing">Phòng Marketing</option> </select> </div> <span className="text-slate-500 font-medium"> Hiển thị <strong className="text-slate-900">{filteredKpis.length} Chỉ Tiêu KPI</strong> </span> </div> {/* KPI Table */} <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden p-6"> <div className="overflow-x-auto"> <table className="w-full text-left border-collapse"> <thead> <tr className="border-b border-slate-200 text-slate-500 font-semibold uppercase text-[10.5px]"> <th className="p-3">Mục Tiêu & Chỉ Tiêu KPI</th> <th className="p-3">Cấp Độ & Đối Tượng</th> <th className="p-3">Chỉ Số Định Mức (Target)</th> <th className="p-3">Tiến Độ Thực Hiện</th> <th className="p-3 text-center">Tỷ Lệ Đạt</th> <th className="p-3 text-center">Thao Tác</th> </tr> </thead> <tbody className="divide-y divide-slate-100"> {filteredKpis.map((kpi) => {
                    const pct = Math.round(((kpi.actual_value || 0) / (kpi.target_value || 1)) * 100);
                    return ( <tr key={kpi.id} className="hover:bg-slate-50 transition-colors"> <td className="p-3"> <p className="font-semibold text-slate-900 text-sm">{kpi.kpi_name}</p> <p className="text-slate-500 text-[11px]">{kpi.notes || kpi.period}</p> </td> <td className="p-3"> <p className="font-medium text-slate-800">{kpi.assignee_name}</p> <span className="px-2 py-0.5 rounded bg-slate-100 text-slate-600 font-medium text-[10px]"> {kpi.assignee_type === 'Company' ? '🏢 Công ty' : kpi.assignee_type === 'Department' ? '🏬 Phòng ban' : '👤 Cá nhân'} </span> </td> <td className="p-3 font-mono font-medium text-slate-900"> {formatNumberWithUnit(kpi.target_value, kpi.unit)} </td> <td className="p-3 font-mono font-medium text-blue-700"> {formatNumber(kpi.actual_value || 0)} / {formatNumberWithUnit(kpi.target_value, kpi.unit)} </td> <td className="p-3 text-center"> <span className={`px-2.5 py-1 rounded-full font-semibold text-[11px] ${
                          ...
                          }`}> {pct}% </span> </td> <td className="p-3 text-center"> <div className="flex items-center justify-center gap-1.5"> <button
                              onClick={() => handleOpenDetailKpiModal(kpi)}
                              className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium rounded-lg"
                            > Chi Tiết </button> <button
      ...
      {activeMainTab === 'SCORECARDS' && ( <div className="space-y-4 text-xs font-medium"> {/* Scorecard Control Bar */} <div className="bg-white p-4 rounded-xl border border-slate-200/80 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4"> <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto"> <select
                value={selectedPeriod}
                onChange={(e) => setSelectedPeriod(e.target.value)}
                className="px-3 py-2 bg-blue-50 border border-blue-200 rounded-xl font-semibold text-blue-800 text-xs"
              > <option value="Tháng 07/2026">Đợt Chấm: Tháng 07/2026</option> <option value="Tháng 06/2026">Đợt Chấm: Tháng 06/2026</option> <option value="Tháng 05/2026">Đợt Chấm: Tháng 05/2026</option> </select> <div className="relative w-full sm:w-64"> <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /> <input
                  type="text"
              ...
              > <option value="ALL">Tất Cả Xếp Loại</option> <option value="S">Xếp Loại S (Xuất Sắc)</option> <option value="A">Xếp Loại A (Tốt)</option> <option value="B">Xếp Loại B (Đạt)</option> <option value="C">Xếp Loại C (Cần Cải Thiện)</option> </select> </div> <div className="flex items-center gap-2 w-full md:w-auto justify-end"> <button
                onClick={handleAutoOpenPeriod}
                className="px-3.5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 font-semibold rounded-xl flex items-center gap-1.5"
              > <RefreshCw className="w-3.5 h-3.5 text-blue-600" /> Mở Đợt Chấm Tự Động </button> <button
                onClick={() => handleTransitionPeriod('FINAL')}
                className="px-3.5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-xl flex items-center gap-1.5 shadow-md"
              > <Lock className="w-3.5 h-3.5" /> Chốt Sổ Hiệu Suất </button> </div> </div> {/* Scorecards Table */} <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden p-6"> <div className="overflow-x-auto"> <table className="w-full text-left border-collapse"> <thead> <tr className="border-b border-slate-200 text-slate-500 font-semibold uppercase text-[10.5px]"> <th className="p-3">Nhân Sự & Phòng Ban</th> <th className="p-3 text-center">Điểm KPIs (P3)</th> <th className="p-3 text-center">Điểm Tuân Thủ</th> <th className="p-3 text-center">Điểm Thái Độ</th> <th className="p-3 text-center">Tổng Điểm 3P</th> <th className="p-3 text-center">Xếp Loại</th> <th className="p-3 text-center">Trạng Thái Duyệt</th> <th className="p-3 text-center">Thao Tác</th> </tr> </thead> <tbody className="divide-y divide-slate-100"> {filteredScorecards.map((sc) => ( <tr key={sc.id} className="hover:bg-slate-50 transition-colors"> <td className="p-3"> <p className="font-semibold text-slate-900 text-sm">{sc.employee_name}</p> <p className="font-mono text-blue-700 text-[11px]">{sc.employee_code} · {sc.department}</p> </td> <td className="p-3 text-center font-mono font-semibold text-blue-700">{sc.kpi_score}</td> <td className="p-3 text-center font-mono font-medium text-slate-700">{sc.compliance_score}</td> <td className="p-3 text-center font-mono font-medium text-slate-700">{sc.behavior_score}</td> <td className="p-3 text-center font-mono font-semibold text-purple-700 text-sm">{sc.final_score}</td> <td className="p-3 text-center"> <span className={`px-2.5 py-1 rounded-full font-semibold text-[11px] ${
                          sc.rating_grade === 'S'
                        ...
                        }`}> Xếp Loại {sc.rating_grade} </span> </td> <td className="p-3 text-center"> <span className="px-2 py-0.5 rounded-full bg-slate-100 text-slate-700 font-medium text-[10px]"> {sc.status} </span> </td> <td className="p-3 text-center"> <div className="flex items-center justify-center gap-1.5"> <button
                            onClick={() => {
                          ...
                          > <Trash2 className="w-3.5 h-3.5" /> </button> </div> </td> </tr> ))} </tbody> </table> </div> </div> </div> )}
