...
export default function PaystubModal({ isOpen, onClose, payroll, onSendEmail }: PaystubModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 overflow-y-auto print:p-0">
      <div className="bg-white rounded-xl border border-slate-200 shadow-2xl w-full max-w-2xl overflow-hidden my-8 animate-in fade-in zoom-in duration-200 print:shadow-none print:border-none print:m-0 font-sans">
        {/* Header */}
        <div className="bg-slate-900 text-white p-5 flex items-center justify-between print:hidden">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-emerald-600 flex items-center justify-center font-medium text-white shadow-md">
            ...
            <div>
              ...
              <h3 className="font-semibold text-sm text-white">Phiếu Lương Chi Tiết - {payroll.period}</h3>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-5 text-xs max-h-[80vh] overflow-y-auto print:max-h-none print:p-6">
          ...
          <div className="flex items-start justify-between border-b border-slate-200 pb-4">
            ...
            <div className="text-right">
              ...
              <p className="text-[11px] font-semibold text-emerald-700">KỲ THUẾ: {payroll.period}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 p-3 bg-slate-50 border border-slate-200/80 rounded-xl">
            <div>
              <span className="text-slate-500 font-medium block">Mã & Họ Tên Nhân Sự:</span>
              <span className="font-semibold text-slate-900 text-sm block">
                {payroll.employee_name}{' '}
                <span className="text-xs text-blue-600 font-semibold">({payroll.employee_code})</span>
              </span>
            </div>
            <div>
              <span className="text-slate-500 font-medium block">Bộ Phận & Vị Trí Công Việc:</span>
              <span className="font-medium text-slate-800 block">
                {payroll.department} · {payroll.position}
              </span>
            </div>
            <div>
              <span className="text-slate-500 font-medium block">Tài Khoản Ngân Hàng Nhận Lương:</span>
              <span className="font-semibold text-slate-800 block">
                {payroll.bank_name || 'MBBank'} - {payroll.bank_account || '0988888888'}
              </span>
            </div>
            <div>
              <span className="text-slate-500 font-medium block">Trạng Thái Gửi Phiếu Lương:</span>
          ...
          <div className="space-y-3">
            ...
            <table className="w-full text-left border-collapse border border-slate-200 rounded-xl overflow-hidden text-xs">
              <thead>
                <tr className="bg-slate-100 border-b border-slate-200 text-slate-700 font-medium">
              ...
              <tbody className="divide-y divide-slate-100">
                <tr>
                  ...
                  <td className="py-2 px-3 text-right tabular-nums font-semibold text-red-600">
          ...
          <div className="p-4 bg-gradient-to-r from-emerald-600 to-teal-700 text-white rounded-xl shadow-lg flex items-center justify-between">
            <div>
              <span className="text-xs font-medium text-emerald-100 block">LƯƠNG THỰC NHẬN (NET SALARY):</span>
              <span className="text-2xl tabular-nums font-bold text-white mt-0.5 block">
                {formatVND(payroll.net_salary)}
              </span>
            </div>
            <div className="text-right">
              <span className="text-[11px] font-medium text-emerald-100 block">Dự kiến chuyển khoản:</span>
              <span className="text-xs font-semibold text-white">Ngày 05 Hàng Tháng</span>
            </div>
          </div>

          {/* PIN PROTECTION SECURITY NOTICE */}
          <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl flex items-center justify-between text-[11px]">
            <div className="flex items-center gap-2 text-amber-900 font-medium">
              <ShieldCheck className="w-4 h-4 text-amber-600 shrink-0" />
              <span>Bảo Mật PDF: File Phiếu Lương PDF được tự động cài Mã PIN (Số Điện Thoại / Số CCCD cá nhân)</span>
            </div>
          ...
          <div className="hidden print:grid grid-cols-2 gap-4 text-center pt-8">
            <div>
              <p className="font-semibold text-slate-900">NGƯỜI LẬP BẢNG LƯƠNG</p>
              <p className="text-[10px] text-slate-400">(Ký & ghi rõ họ tên)</p>
            </div>
            <div>
              <p className="font-semibold text-slate-900">GIÁM ĐỐC NHÂN SỰ (HRD)</p>
              <p className="text-[10px] text-slate-400">(Ký & ghi rõ họ tên)</p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-200 print:hidden">
            <button
              onClick={handlePrint}
              className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-medium flex items-center gap-1.5 transition-all"
            >
            ...
            {onSendEmail && (
              <button
                ...
                className="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-semibold shadow-lg shadow-emerald-600/30 flex items-center gap-1.5 transition-all active:scale-95"
              >
