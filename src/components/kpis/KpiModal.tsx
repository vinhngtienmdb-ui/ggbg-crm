...
const CATEGORY_OPTIONS: { key: KpiCategory; label: string; unit: string; metricType: KpiMetricType }[] = [
  ...
  { key: 'CONTRACTS', label: '🛍️ Gian Hàng / Hợp Đồng', unit: 'Hợp Đồng', metricType: 'Count' },
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="bg-white rounded-xl border border-slate-200 shadow-xl w-full max-w-2xl overflow-hidden my-8 animate-in fade-in zoom-in duration-200">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 max-h-[80vh] overflow-y-auto">
          ...
          {/* Grid 1: Assignee Level & Specific Target Owner */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                Cấp Phân Bổ (Assignee Type)
              </label>
              <select
                value={formData.assignee_type || 'Department'}
                onChange={(e) => handleAssigneeTypeChange(e.target.value as KpiAssigneeType)}
                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-600"
              >
                <option value="Company">🌐 Toàn Công Ty (Company)</option>
                <option value="Department">🏢 Khối / Bộ Phận / Phòng Ban</option>
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                Đối Tượng Phụ Trách Năng Lực
              </label>
              {formData.assignee_type === 'Company' && (
                <input
                  type="text"
                  readOnly
              ...
              {formData.assignee_type === 'Department' && (
                <select
              ...
              {formData.assignee_type === 'Individual' && (
                <select
                  value={formData.assignee_name || ''}
                  onChange={(e) => {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                Loại Chỉ Số KPI (Metric Category)
              </label>
              <select
          ...
          <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">
                  Mục Tiêu Chỉ Tiêu (Target Value)
                </label>
                <input
                  type="number"
                  min={0}
            ...
            {/* Computed Progress Badge Preview */}
