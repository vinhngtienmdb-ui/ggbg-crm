...
export default function LaborBook({
  ...
  const thCls = 'p-3 uppercase text-slate-500 font-semibold tracking-wide text-[10.5px]';
      ...
      <div className="p-5 border-b border-slate-200 space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="font-semibold text-slate-900 text-base">Sổ Quản Lý Lao Động</h2>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-3.5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-medium flex items-center gap-1.5 shadow-sm transition-all"
            >
              <Download className="w-4 h-4" /> Xuất CSV
            </button>
            <button
              onClick={handlePrint}
              className="px-3.5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-medium flex items-center gap-1.5 transition-colors"
            >
          ...
          <tbody>
            {filtered.map((e) => {
              ...
              return (
                ...
                >
                  <td className="p-3">
                    <p className="font-medium text-slate-900 text-sm">{e.full_name}</p>
                  ...
                  <td className="p-3">
                    <span className="px-2 py-0.5 rounded-md text-[11px] font-semibold bg-blue-50 text-blue-700 border border-blue-200 inline-block">
                      {e.salary_grade || 'G4'} · Bậc {e.salary_step_number || 1}
                    </span>
                    <p className="text-[10px] text-slate-500 font-mono mt-0.5">
                      BHXH: {maskSalary(showPII, formatVND(e.insurance_salary))}
                    </p>
                  </td>
                  <td className="p-3">
                  ...
                  <td className="p-3">
                    <span
                      className={`px-2 py-0.5 rounded-lg text-[10px] font-medium border ${
                        BHXH_BADGES[e.bhxh_status || ''] || 'bg-slate-100 text-slate-600 border-slate-200'
                      }`}
                    >
                      {e.bhxh_status || 'Chưa xác định'}
                    </span>
                    <p className="font-medium text-slate-900 mt-1">{maskSalary(showPII, formatVND(e.base_salary))}</p>
                  </td>
                  <td className="p-3">
                  ...
                  <td className="p-3">
                    <span className={`px-2.5 py-1 border font-medium rounded-xl text-[11px] w-fit inline-block ${statusBadge.cls}`}>
                      {statusBadge.label}
                    </span>
                  </td>
                </tr>
