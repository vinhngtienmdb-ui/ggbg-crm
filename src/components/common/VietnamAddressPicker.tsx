...
interface VietnamAddressPickerProps {
  ...
  onChange: (val: VietnamAddressValue) => void;
}

export default function VietnamAddressPicker({
  label = 'Địa Chỉ Đơn Vị Hành Chính',
  required = false,
  disabled = false,
  value,
  onChange,
  ...
  return ( <div className="space-y-3 p-4 bg-slate-50/90 rounded-xl border border-slate-200/90 text-xs"> {/* Label */} <div className="border-b border-slate-200/80 pb-2"> <label className="font-semibold text-slate-800 flex items-center gap-1.5"> <MapPin className="w-4 h-4 text-red-600" /> <span>{label}</span> {required && <span className="text-red-500">*</span>} </label> </div> {/* Select Grid: Province & Ward */} <div className="grid grid-cols-1 sm:grid-cols-2 gap-3"> {/* Dropdown 1: Tỉnh / Thành Phố */} <div> <label className="block font-medium text-slate-700 mb-1"> Tỉnh / Thành Phố {required && <span className="text-red-500">*</span>}: </label> <select
          ...
          > <option value="">-- Chọn Tỉnh / Thành Phố --</option> {provinces.map((p) => ( <option key={p.code} value={p.code}> {p.type} {p.name} </option> ))} </select> </div> {/* Dropdown 2: Phường / Xã (Sau 01/07/2025) */} <div> <label className="block font-medium text-slate-700 mb-1"> Phường / Xã / Thị Trấn {required && <span className="text-red-500">*</span>}: </label> <select
            disabled={disabled || !selectedProvinceCode || loadingWards}
            value={selectedWardCode}
            onChange={handleWardChange}
            required={required}
            className="w-full px-3 py-2 bg-white border border-slate-300 rounded-xl font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 disabled:bg-slate-100"
          > <option value=""> {!selectedProvinceCode
                ? '-- Chọn Tỉnh trước --'
                : loadingWards
                ? 'Đang tải danh sách Phường/Xã...'
                : '-- Chọn Phường / Xã --'} </option> {wards.map((w) => ( <option key={w.code} value={w.code}> {w.type} {w.name} </option> ))} </select> </div> </div> {/* Input 3: Số nhà, Tên đường... */} <div> <label className="block font-medium text-slate-700 mb-1"> Địa Chỉ Chi Tiết (Số nhà, Tên đường, Tòa nhà...) {required && <span className="text-red-500">*</span>}: </label> <input
          type="text"
          disabled={disabled}
          value={detailAddress}
